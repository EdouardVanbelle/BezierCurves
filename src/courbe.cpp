...
if (nbr_pts>1)
{
 if (input ("Entrez le nom de sauvegarde:",buffer))
 {
  if ((f=fopen(buffer,"wt"))==NULL)
   {
    erase_bar_bas();
    echo ("Erreur de création de fichier...",RED);
    return(FALSE);
   }
  else
   {
    for (i=0;i<nbr_pts;i++)
     if (EOF==fprintf (f,"%d,%d\n",(int)(p[0][i].x),(int)(p[0][i].y)))
      {
       erase_bar_bas();
       echo ("Une erreure est survenue durant l'écriture du fichier",RED);
       fclose (f);
       return (FALSE);
      }
    ...
    fclose (f);
    erase_bar_bas();
    echo ("Ecriture sur disque achevée.");
    return (intact=TRUE);
   }
 }
 erase_bar_bas();
  ...
  while ((fscanf(f,"%s",txt)!=EOF) && (!sortie))
   {
    ...
    {
     case 0:
              sprintf (buffer,"Erreur à la ligne %d: %s\n",ligne,txt);
              echo (buffer,RED);
     ...
     default:
              if (nbr_pts==PMAX)
              {
               sprintf (buffer,"Version étudiante limité à %d points.",PMAX);
               echo (buffer,RED);
...
do
{
  ...
  if (nbr_pts==PMAX-1)
   ...
   {
    aff_controle ();
    sprintf (buffer,"Indiquez les coordonnées du points n°%d",p_sel+1);
    echo (buffer);
    if (nbr_pts!=0)
    {
     if (p_sel==0)
 ...
 do
 {
  ...
  if (keypressed())
  {
   ...
   default   :
    ...
    update_coor=TRUE;
    erase_bar_bas();
    input ("Entrez les coordonnées au format ###,### :",buffer);
...
if (!nbr_pts)
 {
  erase_bar_bas();
  echo ("Il n'y a pas de courbe chargée...",RED);
  return;
 }
   ...
   if ((x!=xx) || (y!=yy))
    {
     ...
     erase_bar_bas();
     sprintf (buffer,"Similitude de centre Û(%.3d,%.3d), de rapport k=%f et d'angle Ó=%+f",
...
if (nbr_pts)
{
 ...
 outtextxy (DEP,25,"déplacer  le point. ",MAGENTA);
 ...
 if (contexte2==SAISIE)
  {
   ...
   else
   {
    if (old_pnt!=find_pnt)
    {
     ...
     sprintf (buffer,"Point sélétionné: %d",find_pnt+1);
     erase_bar_bas();
     echo (buffer);
    } else
    {
...
void degre (void)
{
 ...
 if (accept())
 {
  ...
  {
   erase_bar_bas();
   echo ("Calcul des nouveaux points pour une élévation de un degré...");
...
void envel (void)
{
 ...
 erase_bar_bas();
 mouse_cache();
 outtextxy (1,29,"Il n'y a pas de courbe chargée...",RED);
 mouse_affiche();
 }
}
